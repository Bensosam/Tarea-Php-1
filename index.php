...
/*
 * Mensaje flash (registrado, modificado, eliminado, error, etc.)
 */
$mensaje = obtener_mensaje();

/*
 * Token CSRF para el formulario de agregar / modificar / eliminar.
 */
...
$por_pagina = 8; // Entre 5 y 10 empleados por página.
...
if ($buscar !== "") {
    ...
    $sql_conteo = "SELECT COUNT(*) AS total
                    ...
                    WHERE e.codigo LIKE ?
                       ...
                       OR e.telefono LIKE ?
                       OR p.puesto LIKE ?";
    $consulta_conteo = $conexion->prepare($sql_conteo);
    $consulta_conteo->bind_param("sssss", $like, $like, $like, $like, $like);
    $consulta_conteo->execute();
    $total_registros = (int) $consulta_conteo
        ->get_result()
        ->fetch_assoc()["total"];
    $consulta_conteo->close();
} else {
    $total_registros = (int) $conexion
...
/*
 * Cargar puestos (para el <select> del formulario).
 */
$sql_puestos = "SELECT id_puesto, puesto
                FROM puestos
                ORDER BY puesto";
$resultado_puestos = $conexion->query($sql_puestos);

/*
 * Cargar empleados de la página actual.
 */
$sql_base = "SELECT
                e.id_empleado,
                e.codigo,
...
if ($buscar !== "") {
    $sql_empleados = $sql_base
        . " WHERE e.codigo LIKE ?
            ...
            LIMIT ? OFFSET ?";
    $consulta_empleados = $conexion->prepare($sql_empleados);
    $consulta_empleados->bind_param(
        "sssssii",
        $like,
        $like,
        $like,
    ...
    $sql_empleados = $sql_base
        . " ORDER BY e.id_empleado DESC
            LIMIT ? OFFSET ?";
    $consulta_empleados = $conexion->prepare($sql_empleados);
    $consulta_empleados->bind_param("ii", $por_pagina, $offset);
}
...
/*
 * Helper para construir los enlaces de paginación conservando
 * el término de búsqueda.
 */
function url_pagina(int $pagina, string $buscar): string
{
...
function formatear_fecha_hora(?string $valor): string
{
    if (empty($valor)) {
        return "";
    }
    ...
    return escapar($fecha->format("d/m/Y H:i"));
}
...
<div class="container mb-4">
    <div
        class="alert alert-<?= escapar($mensaje["tipo"]) ?> alert-dismissible fade show"
        role="alert"
    >
                    ...
                    <tbody id="tbl_empleados">
                        <?php if ($resultado_empleados->num_rows > 0): ?>
                            ...
                            >
                                ...
                                <td><?= formatear_fecha_hora($fila["created_at"]) ?></td>
                                <td><?= formatear_fecha_hora($fila["updated_at"]) ?></td>
                            </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr id="fila_sin_registros">
                                <td colspan="9" class="text-center text-secondary py-4">
                                    <?php if ($buscar !== ""): ?>
                                        No se encontraron empleados para
                                        "<?= escapar($buscar) ?>".
                                    <?php else: ?>
        ...
        <div class="card-footer bg-white">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
                <div class="text-secondary small">
                    ...
                    Total de registros: <strong><?= (int) $total_registros ?></strong>
                </div>
                <nav aria-label="Paginación de empleados">
                    <ul class="pagination pagination-sm mb-0">
                        <li class="page-item <?= $pagina_actual <= 1 ? "disabled" : "" ?>">
                            <a
                                class="page-link"
                                href="<?= escapar(url_pagina($pagina_actual - 1, $buscar)) ?>"
                            >
                                Anterior
                            </a>
                        </li>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="invalid-feedback">
                                Ingrese solamente letras y espacios para los nombres.
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label
                            ...
                            <div class="invalid-feedback">
                                Ingrese solamente letras y espacios para los apellidos.
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="invalid-feedback">
                                Seleccione una fecha de nacimiento válida (no puede ser futura).
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label
